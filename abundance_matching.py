...
g09 = Region(
    area=59.978679332, random_file_name="gama_g09_randoms.txt", ra_range=(129, 141)
)
g12 = Region(
    area=59.978679332, random_file_name="gama_g12_randoms.txt", ra_range=(174, 186)
)
g15 = Region(
    area=59.978679332, random_file_name="gama_g15_randoms.txt", ra_range=(211.5, 223.5)
)
g23 = Region(
    area=50.587431294, random_file_name="gama_g23_randoms.txt", ra_range=(339, 351)
)
...
for lightcone in lightcones:
    for region in regions:
        ...
        nz_random = (
            np.histogram(df_randoms["z"], bins=z_bin, density=False)[0]
            / RANDOM_OVERFACTOR
        )
        ...
        for i in range(len(z_bin) - 1):
            z_selection = (df_shark["zobs"] > z_bin[i]) & (
                df_shark["zobs"] <= z_bin[i + 1]
            )
            number_in_bin = len(np.where(z_selection is True)[0])
            if number_in_bin > 100:  # This seems arbitary
                shark_mags = np.sort(
                    df_shark.loc[z_selection, "total_ap_dust_r_SDSS"].to_numpy(
                        copy=True
                    )
                )
                n_selection = np.sum(shark_mags <= GAMA_MAG_LIMIT)
                idx = int(
                    np.min([np.round(n_selection * nz_ratio[i]), number_in_bin - 1])
                )
        ...
        df_shark["total_ap_dust_r_SDSS_matched"] = df_shark[
            "total_ap_dust_r_SDSS"
        ] + mag_diff_spline(df_shark["zobs"])
        ...
        plt.hist(
            df_shark["zobs"], bins=z_bin, histtype="step", label="Not matched SHARK"
        )

...
# This is here only because FoFempint expects this as input, eventually we should rework the FoFempint code
...
CalCat = CalCat[Y_VISTA_ap_dust_total_magcorr < Ylim, ]
CalCat = CalCat[completeness_selected == 1, ]
...
optimFoFfunc = function(par, data){
    ...
    FoFout = foreach(LC = 1:32) %dopar% {
        ...
        if (file.exists(precalc_file)) {
            ...
            PreCalc_p1090 = FoFempint(data = cat_subset, bgal = bgal, rgal = rgal, Eb = Eb, Er = Er, coscale = T,
        ...
        out = tryCatch({catGroup = FoFempint(data = cat_subset, bgal = bgal, rgal = rgal, Eb = Eb, Er = Er, coscale = T,
                                             NNscale = 20, groupcalc = F, precalc = T, halocheck = T, apmaglim = Ylim,
                                             denfunc = LFswmlfunc, colnames = c('RA', 'Dec', 'zobs',
                        ...
                        list(mockfrac_num = catGroup$summary['mockfrac_num'],
                       ...
                       error = function(err){return(list(mockfrac_num = 0, mockfrac_den = 0, foffrac_num = 0,
...
selectz_p1090 = CalCat$zobs > 0.186 & CalCat$zobs < 0.842
CalData_p1090 = list(maincat = CalCat[selectz_p1090, ])
...
optDEVILS_p1090 = Highlander(optB24, Data = CalData_p1090, likefunc = optimFoFfunc, likefunctype = 'CMA',
...
print(paste("Final parameters = ", optDEVILS_p1090$parm[1], ", ", optDEVILS_p1090$parm[2], ", ", 
            ...
            optDEVILS_p1090$parm[6], ", ", optDEVILS_p1090$parm[7], sep=""))
